package com.jerry.ci.plugins.gssh;
...
import com.jerry.ci.plugins.gssh.client.SshClient;
...
public final class GsshBuilderWrapper extends BuildWrapper {
	...
	private void executePreBuildScript(PrintStream logger) {
		...
		if (preScript != null && !preScript.trim().equals("")) {
			sshHandler.executeShellByFTP(logger, preScript);
		}
		printSplit(logger);
	}
	private void executePostBuildScript(PrintStream logger) {
		...
		if (postScript != null && !postScript.trim().equals("")) {
			sshHandler.executeShellByFTP(logger, postScript);
		}
		printSplit(logger);
	}
	...
	public static final class GsshDescriptorImpl extends BuildWrapperDescriptor {
		public static final Logger LOGGER = Logger
